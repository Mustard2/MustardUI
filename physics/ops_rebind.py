                    ...
                    with bpy.context.temp_override(object=obj):
                        try:
                            ...
                            if addon_prefs.debug:
                                print(obj.name + " modifier: " + m.name + " could not be bound")
                    with bpy.context.temp_override(object=obj):
                        try:
                            bpy.ops.object.surfacedeform_bind(modifier=m.name)
                        except Exception:
    ...
    def execute(self, context):
        ...
        if rig_settings.hair_collection is not None:
            for obj in [x for x in rig_settings.hair_collection.objects if x.type == "MESH"]:
                objects.append(obj)
        for item in [
            x for x in physics_settings.items if x.type == "COLLISION" and x.object is not None
        ]:
...
class MustardUI_PhysicsItem_Rebind_Outfit(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"},
            f'MustardUI - Cages successfully re-bound for Outfit "{self.outfit}".',
        )
...
class MustardUI_PhysicsItem_Rebind_Single(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"},
            f'MustardUI - Cages successfully re-bound to "{self.object_name}".',
        )
...
class MustardUI_PhysicsItem_Rebind_SingleCage(bpy.types.Operator):
    """Rebind one cage to the model meshes (Body, Outfits, Hair).\nDepending on the number of Objects, Blender might freeze for a while"""  # noqa: E501
    ...
    def execute(self, context):
        ...
        for item in [
            ...
            if x.type == "CAGE" and x.object is not None and x.object.name == self.cage_name
        ]:
            cages.append(item.object)
            break
