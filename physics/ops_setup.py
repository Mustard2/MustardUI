...
fixes = [
    ...
    (
        "SUBDIVISION",
        "Subdivision",
        "Increase the Subdivision modifier level and re-attempt a binding",
    ),
    (
        "CONCAVE_SPLIT",
        "Concave Split",
        "Use the Concave Split operator before re-attempting a binding",
    ),
]


class MustardUI_Physics_Setup(bpy.types.Operator):
    """This button creates Surface Deform modifiers on Outfit pieces affected by
    Cages physics items.\nThe modifier is added only if the Outfit piece and the
    ...
    attempt_fix_bind: bpy.props.EnumProperty(
        items=fixes,
        default="NONE",
        name="Attempt Fix Binding",
        description="Attempt to fix the rebinding if errors occur "
        "(Simplify should be off).\nTo use the model with these fixes, read carefully "
        "the warnings if generated",
    )
    override_armature_check: bpy.props.BoolProperty(
        default=False,
        name="Override Armature Check",
        description="Enable this to add Surface Deform modifiers even if the Armature "
        "modifier is not available on the Object",
    )
    override_physics_check: bpy.props.BoolProperty(
        default=False,
        name="Override Physics Check",
        description="Enable this to add Surface Deform modifiers even if Physics "
        "modifiers are found on the Object",
    )
    ...
    def bind(self, obj, mod):
        ...
        if not mod.is_bound and self.attempt_fix_bind:
            print(
                f"MustardUI Physics Setup - Surface Deform might be fixed "
                f"on {repr(obj.name)}, but will require Subdivision Surface on the "
                f"Body to work properly"
            )
            warnings += 1
    ...
    def execute(self, context):
        ...
        for coll in colls:
            ...
            for obj in [
                ...
                for mod in [
                    ...
                    if not mod.is_bound:
                        print(
                            f"MustardUI Physics Setup - Could not bind Surface Deform "
                            f"on {repr(obj.name)}"
                        )
                        warnings += 1
        ...
        if warnings == 0:
            ...
            self.report(
                {"WARNING"},
                f"MustardUI - {str(warnings)} warnings have been generated during "
                f"setup. Check the Console log.",
            )
...
class MustardUI_Physics_Setup_Clear(bpy.types.Operator):
    """Clear the data driving the enable/disable of surface deform modifiers when
    Physics are activated/disabled"""
