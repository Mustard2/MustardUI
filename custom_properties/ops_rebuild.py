...
class MustardUI_Property_Rebuild(bpy.types.Operator):
    """Rebuild all drivers and custom properties. This can be used if the properties
    aren't working or if the properties max/min/default/descriptions are broken"""
    ...
    def execute(self, context):
        ...
        if errors > 0:
            if errors > 1:
                ...
                self.report(
                    {"WARNING"},
                    "MustardUI - A custom property was corrupted and deleted. "
                    "Check the console for more infos.",
                )
        else:
            self.report(
