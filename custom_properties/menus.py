    ...
    def draw(self, context):
        ...
        for obj in items:
            op = layout.operator(
                ...
                icon="DOT",
                text=obj.name[
                    len(context.mustardui_propertyoutfitmenu_sel.name + " - ") :
                ]
        ...
        for i in outfit_indices:
            ...
            layout.menu(
                ...
                text=rig_settings.outfits_collections[i].collection.name[
                    len(rig_settings.model_name) :
                ]
                if rig_settings.model_MustardUI_naming_convention
                else rig_settings.outfits_collections[i].collection.name,
            )
        if rig_settings.extras_collection is not None:
            ...
            if len(items) > 0:
                ...
                layout.menu(
                    ...
                    if rig_settings.model_MustardUI_naming_convention
                    else rig_settings.extras_collection.name,
                )
    ...
    def draw(self, context):
        ...
        for obj in [
            ...
            op = layout.operator(
                MustardUI_Property_MenuAdd.bl_idname,
                icon="STRANDS",
                text=obj.name[len(rig_settings.hair_collection.name) :]
                if rig_settings.model_MustardUI_naming_convention
                else obj.name,
            )
            op.section = ""
    ...
    def draw(self, context):
        ...
        for prop in sorted(
            ...
            if prop.outfit_piece is not None:
                outfit_piece_name = (
                    prop.outfit_piece.name[len(prop.outfit.name + " - ") :]
                    if rig_settings.model_MustardUI_naming_convention
                    else prop.outfit.name
                )
        ...
        for prop in sorted(extras_props, key=lambda x: x.name):
            outfit_name = (
                prop.outfit_piece.name[
                    len(rig_settings.extras_collection.name + " - ") :
                ]
                if rig_settings.model_MustardUI_naming_convention
                else prop.outfit_piece.name
            )
            op = layout.operator(
                MustardUI_Property_MenuLink.bl_idname,
                text=outfit_name + " - " + prop.name,
        ...
        for prop in sorted(hair_props, key=lambda x: x.name):
            if prop.hair is not None:
                hair_name = (
                    prop.hair.name[len(rig_settings.hair_collection.name + " ") :]
                    if rig_settings.model_MustardUI_naming_convention
                    else prop.hair.name
                )
                op = layout.operator(
