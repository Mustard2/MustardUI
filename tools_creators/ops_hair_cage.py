...
class MustardUI_ToolsCreators_HairCage(bpy.types.Operator):
    ...
    max_density: bpy.props.BoolProperty(
        ...
        description="Enable Subdivision during the process. Might get better results, "
        "but it is slower",
        default=False,
    )
    attempt_tight_bind: bpy.props.BoolProperty(
        name="Attempt Tight Binding",
        description="Add a Displace modifier to the cage to attempt a tighter binding",
        default=True,
    )
    voxel_res: bpy.props.FloatProperty(
    ...
    def execute(self, context):
        ...
        if len(bpy.context.selected_objects) > 1 and bpy.context.active_object:
            ...
            print(
                "Make sure there are at least two selected objects and one "
                "active object."
            )
        ...
        def select_inward_facing_faces():
            """Selects inward-facing faces and deletes them, keeping the largest
            connected region."""
            ...
            if selected_regions:
                largest_region = max(
                    selected_regions, key=lambda reg: sum(f.calc_area() for f in reg)
                )
                bpy.ops.mesh.select_all(action="DESELECT")
        ...
        def create_gradient_vertex_group(obj, group_name):
            """Creates a vertex group with a gradient weight based on the Z position
            of vertices."""
            vertex_group = obj.vertex_groups.new(name=group_name)
            bbox_min_z = min(
                [obj.matrix_world @ v.co for v in obj.data.vertices], key=lambda v: v.z
            ).z
            bbox_max_z = max(
                [obj.matrix_world @ v.co for v in obj.data.vertices], key=lambda v: v.z
            ).z
        ...
        def transfer_armature_modifiers(original_obj, duplicate_obj):
            """Copies all armature modifiers from the original object to
            the duplicate."""
        ...
        def perform_weight_transfer(original_obj, duplicate_obj):
            """Performs vertex group weight transfer from the original object
            to the duplicate."""
        ...
        def apply_corrective_smooth_modifier(obj, iterations, smooth_type, rest_source):
            """Applies a corrective smooth modifier with the given settings and
            binds it."""
        ...
        surface_deform_modifier = original_obj.modifiers.new(
        ...
        apply_corrective_smooth_modifier(
            duplicate_obj, iterations=10, smooth_type="SIMPLE", rest_source="BIND"
        )
        ...
        if addon_prefs.debug:
            print(
                "All operations completed: Proxy creation, armature transfer, "
                "Surface Deform binding, corrective smooth application, and "
                "weight projection with active vertex group 'Hair Pin'."
            )
        ...
        if active_obj is None or active_obj.type != "MESH":
            ...
            # Find the mesh that uses the active object as a target in a Surface
            # Deform modifier
            target_obj = None
            for obj in bpy.data.objects:
            ...
            if target_obj is None:
                ...
                if surface_deform_modifier:
                    ...
                    if addon_prefs.debug:
                        print(
                            f"Assigned '{vg_name}' to the last Surface Deform modifier "
                            f"of the target object."
                        )
                else:
                ...
                if corrective_smooth_modifier:
                    ...
                    if addon_prefs.debug:
                        print(
                            f"Assigned '{vg_name}' to the last Corrective Smooth "
                            f"modifier of the target object."
                        )
                else:
