...
class MustardUI_ToolsCreators_HairCage(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def select_inward_facing_faces():
            """Selects inward-facing faces and deletes them, keeping the largest connected region."""  # noqa: E501
        ...
        def create_gradient_vertex_group(obj, group_name):
            """Creates a vertex group with a gradient weight based on the Z position of vertices."""  # noqa: E501
        ...
        def perform_weight_transfer(original_obj, duplicate_obj):
            """Performs vertex group weight transfer from the original object to the duplicate."""  # noqa: E501
        ...
        if addon_prefs.debug:
            print(
                "All operations completed: Proxy creation, armature transfer, Surface "
                "Deform binding, corrective smooth application, and weight projection "
                "with active vertex group 'Hair Pin'."
            )
        ...
        if active_obj is None or active_obj.type != "MESH":
            ...
            # Find the mesh that uses the active object as a target in a
