...
class MustardUI_ToolsCreators_CreateJiggle_Preset(bpy.types.Operator):
    ...
    bl_description = (
        "Adds cloth jiggle physics with Wildeer settings to mesh."
        "Uses active vtx group as 'Pin Group' in object mode. If in edit mode then it "
        "creates a new vtx group from selection and uses it instead. Applies to active "
        "and selected"
    )
...
class MustardUI_ToolsCreators_CreateJiggle(bpy.types.Operator):
    ...
    bl_description = (
        "Needs to select vertices in Edit Mode.\nCreates a jiggle cage"
        "using the selected regions in Edit Mode and attaches it to the active mesh"
    )
    ...
    def execute(self, context):
        ...
        def create_gradient_vertex_group(obj, group_name, all_islands):
            """Creates a vertex group with a gradient weight based on the Y position of
            vertices for all islands."""
