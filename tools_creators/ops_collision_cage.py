...
class MustardUI_ToolsCreators_CreateCollisionCage(bpy.types.Operator):
    """Create a collision cage by duplicating the current mesh and masking it based on
    the selection in Edit Mode. If nothing is selected or if the mesh is in Object Mode,
      the entire mesh is used.\nThe inflation value of the cage can be adjusted via the
      object's 'Inflate' custom property.\nThe operation applies to both the active
      object and any selected objects."""
    ...
    def execute(self, context):
        ...
        def remove_drivers_from_visibility(obj):
            """Remove drivers from 'Disable in Viewport' and 'Disable in Renders'
            properties."""
        ...
        def remove_subdiv_and_multires_modifiers(obj):
            """Remove all Subdivision Surface and Multiresolution modifiers from the
            object."""
        ...
        def create_proxy_for_object(obj, vertex_selection_required):
            """Create a proxy object for the given object with specified modifiers and
            drivers."""
            ...
            print(
                f"Duplicate proxy created for {obj.name} with materials unlinked and "
                f"link set to 'OBJECT'."
            )

            return duplicate_obj

        def process_proxy(proxy, edit_mode_with_selection):
            """Process each proxy individually with different operations based on edit
            mode and selection state."""
            bpy.context.view_layer.objects.active = proxy
            proxy.select_set(True)
            bpy.ops.object.mode_set(mode="EDIT")

            # Ensure there is a valid selection before deleting to avoid
        ...
        # Determine if we have vertex selections in Edit mode for each object
        # individually
        ...
        print(
            "Collision modifier applied to all selected meshes, and selection state "
            "restored."
        )
