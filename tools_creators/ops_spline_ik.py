...
class MustardUI_ToolsCreators_IKSpline(bpy.types.Operator):
    ...
    ik_spline_resolution: bpy.props.IntProperty(
        default=32,
        min=1,
        max=64,
        name="Resolution",
    ...
    ik_spline_bendy_segments: bpy.props.IntProperty(
        name="Segments",
        default=2,
        min=2,
        max=32,
    ...
    def execute(self, context):
        ...
        if self.ik_spline_number > len(chain_bones) - 1:
            self.report(
                {"WARNING"},
                "MustardUI - The number of bones selected can not be smaller than the "
...
class MustardUI_ToolsCreators_IKSpline_Clean(bpy.types.Operator):
    ...
    delete_bones: bpy.props.BoolProperty(
        name="Delete bones",
        description="Delete controller and pole bones",
        default=True,
    )
    reset_bendy: bpy.props.BoolProperty(
        name="Reset Bendy Bones",
