...
data = [
    ...
    (
        "Mouth_Shrug_Upper",
        ["facs_bs_MouthShrugUpperLeft", "facs_bs_MouthShrugUpperRight"],
        ["Z_UP"],
        100,
        0.01,
    ),
    (
        "Mouth_Shrug_Lower",
        ["facs_bs_MouthShrugLowerLeft", "facs_bs_MouthShrugLowerRight"],
        ["Z_UP"],
        100,
        0.01,
    ),
    ...
    (
        "l_Nose",
        ["facs_bs_NoseSneerLeft", "facs_bs_NoseSneerUpperLeft"],
        ["Z_UP"],
        100,
        0.01,
    ),
    (
        "r_Nose",
        ["facs_bs_NoseSneerRight", "facs_bs_NoseSneerUpperRight"],
        ["Z_UP"],
        100,
        0.01,
    ),
    (
        "Nostrils_Dilate",
        ["facs_bs_NasalFlareLeft", "facs_bs_NasalFlareRight"],
        ["X_DOWN"],
        100,
        0.01,
    ),
    ...
    (
        "l_Mouth_Corner",
        ["facs_bs_MouthCompressLowerLeft", "facs_bs_MouthCompressUpperLeft"],
        ["Y_UP"],
        100,
        0.01,
        4,
    ...
    (
        "r_Mouth_Corner",
        ["facs_bs_MouthCompressLowerRight", "facs_bs_MouthCompressUpperRight"],
        ["Y_UP"],
        100,
        0.01,
        4,
    ...
    (
        "Mouth_Press_Top",
        ["facs_bs_MouthRollUpperLeft", "facs_bs_MouthRollUpperRight"],
        ["Z_DOWN"],
        100,
        0.01,
        3,
    ...
    (
        "Mouth_Press_Bottom",
        ["facs_bs_MouthRollLowerLeft", "facs_bs_MouthRollLowerRight"],
        ["Z_UP"],
        100,
        0.01,
        3,
...
def compute_fixes(driver, var_out, armature, bone, sk, fac):
    expression = ""
    if bone == "Mouth_Close" and (
        "facs_bs_MouthCloseLowerLeft" in sk
        or "facs_bs_MouthCloseLowerRight" in sk
        or "facs_bs_MouthCloseUpperLeft" in sk
        or "facs_bs_MouthCloseUpperRight" in sk
    ):
        # Create a new variable for the driver
        var = driver.variables.new()
        var.name = "fcd_fix"  # Name for the variable
        var.type = "TRANSFORMS"
...
class MustardUI_ToolsCreators_FaceController(bpy.types.Operator):
    """Add a Face Controller rig to the model.\nIf another controller is available on
    the model, the tool can not is disabled"""
    ...
    add_to_armature_panel: bpy.props.BoolProperty(
        default=True, name="Add Collection to Armature Panel"
    )
    ...
    def execute(self, context):
        ...
        if not os.path.exists(blend_file):
            self.report(
                {"ERROR"},
                "MustardUI - An error occurred while appending the face "
                "controller rig.",
            )
            return {"CANCELLED"}
        ...
        try:
            ...
            if not head_bone or not face_controls_bone:
                ...
                bpy.ops.object.mode_set(
                    mode="OBJECT"
                )  # Switch back to object mode if an error occurs
                return {"CANCELLED"}
                    ...
                    for dir in dirs:
                        ...
                        for dr in model_armature.data.animation_data.drivers:
                            if dr.data_path == f'["{sk}(fin)"]':
                                # We found the existing driver for this shape key,
                                # so use it
                                driver = dr.driver
                                break

                        if driver is None:
                            # No driver exists, so add one for the shape key's value
                            # property
                        ...
                        variables_to_remove = [
                            ...
                            if f"fcd{str(num)}" in var.name or "fcd_fix" in var.name
                        ]
                        ...
                        var.targets[
                            ...
                            direction  # Transformation type (e.g., LOC_X, LOC_Y, LOC_Z)
                        )
                        var.targets[
                            0
                        ].transform_space = (
                        ...
                        term_to_add = (
                            ...
                            if "SCALE" not in direction
                            else f"{sign}{fac}*(1-fcd{str(num)})"
                        )
                        if fixed_expression != "":
                            term_to_add = fixed_expression
                        if term_to_add not in current_expression:
                            # Append the term only if it doesn't already exist in
                            # the expression
            ...
            if addon_prefs.debug:
                self.report(
                    {"INFO"},
                    "MustardUI - Controller successfully added to the model "
                    "(controls added: " + str(ctrl_added) + ").",
                )
            else:
                self.report(
        ...
        for dr in model_armature.data.animation_data.drivers:
            ...
            for var in driver.variables:
                ...
                for target in var.targets:
                    if any(
                        target.id == model_armature and bone == target.bone_target
                        for bone in fctrl_bones
                    ):
