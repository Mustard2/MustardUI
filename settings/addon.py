...
class MustardUI_Settings(bpy.types.PropertyGroup):
    ...
    # Model selection
    viewport_model_selection: BoolProperty(
        default=True,
    ...
    viewport_model_selection_after_configuration: BoolProperty(
        default=False,
        name="Viewport model selection",
        description="Enable viewport model selection "
    ...
    daz_morphs_error: BoolProperty(
        name="",
        description="Can not find the Daz Morph.\nRe-run the Check Morphs operator in "
        "the Configuration menu to solve this",
    )
    ...
    material_normal_nodes: BoolProperty(
        default=False,
        name="EEVEE Optimized Normals",
    ...
    rename_outfits_temp_class: CollectionProperty(type=MustardUI_RenameOutfit_Class)


# Handler to solve a missing UI when: developer mode is on, but the UI has been left in
# configuration mode
@persistent
def load_handler(_):
    context = bpy.context
