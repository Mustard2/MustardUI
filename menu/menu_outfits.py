...
class PANEL_PT_MustardUI_Outfits(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if rig_settings.outfits_list != "Nude":
            ...
            items = outfit_extract_items_from_collection(collection, rig_settings.outfit_config_subcollections)
            ...
            if len(items) > 0:
                ...
                if len(custom_properties) > 0:
                    ...
                    if rig_settings.outfit_global_custom_properties_collapse:
                        mustardui_custom_properties_print(arm, settings, custom_properties, layout,
                ...
                for obj in sorted(items, key=lambda x: x.name):
                    draw_outfit_piece(layout, obj, arm, rig_settings, physics_settings, settings, 0, 0)
...
class PANEL_PT_MustardUI_Outfits_Optimize(MainPanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        gloabal_properties_avail = (
                rig_settings.outfits_enable_global_subsurface or
                rig_settings.outfits_enable_global_smoothcorrection or
                rig_settings.outfits_enable_global_shrinkwrap or
                rig_settings.outfits_enable_global_surfacedeform or
                rig_settings.outfits_enable_global_mask or
                rig_settings.outfits_enable_global_solidify or
