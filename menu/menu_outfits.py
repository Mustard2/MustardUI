...
class PANEL_PT_MustardUI_Outfits(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (
            ...
            if len(locked_objects) > 0:
                ...
                for obj in locked_objects:
                    draw_outfit_piece(
                        box, obj, arm, rig_settings, physics_settings, settings, 1, 0
                    )

        # Extras
        if rig_settings.extras_collection is not None:
            eitems = extract_items(
            ...
            if len(eitems) > 0:
                ...
                if ui_collapse_prop(
                    box, rig_settings, "extras_collapse", "Extras", icon="", align=False
                ):
        ...
        if (
            ...
            or rig_settings.outfits_enable_global_normalautosmooth
        ):
            box = layout.box()
            row = box.row(align=True)
            row.label(text="Global Properties", icon="MODIFIER")
            row.operator(
                "mustardui.switchglobal_outfits", text="", icon="RESTRICT_VIEW_OFF"
            ).enable = True
            row.operator(
                "mustardui.switchglobal_outfits", text="", icon="RESTRICT_VIEW_ON"
            ).enable = False
