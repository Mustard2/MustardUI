...
def cloth_panel(layout, pi, mod):
    if ui_collapse_prop(
        layout, pi, "collapse_cloth", "Cloth settings", icon="MOD_CLOTH"
    ):
        cloth = mod.settings
        box = layout.box()
        ...
        if ui_collapse_prop(
            box, pi, "collapse_cloth_internal_springs", "Internal Springs"
        ):
            col = box.column(align=True)
            ...
            col.prop(
                cloth, "internal_compression_stiffness_max", text="Max Compression"
            )
        ...
        if ui_collapse_prop(box, pi, "collapse_cloth_cache", "Cache"):
            ...
            row.prop(
                pi, "unique_cache_frames", icon="TRACKING_REFINE_BACKWARDS", text=""
            )
        ...
        row.prop(
            pi,
            "collapse_cloth_collisions",
            icon="TRIA_DOWN" if not pi.collapse_cloth_collisions else "TRIA_RIGHT",
            icon_only=True,
            emboss=False,
        )
        ...
        row.prop(
            pi,
            "collapse_cloth_self_collisions",
            icon="TRIA_DOWN" if not pi.collapse_cloth_self_collisions else "TRIA_RIGHT",
            icon_only=True,
            emboss=False,
        )
        ...
        if not pi.collapse_cloth_self_collisions:
            ...
            col.prop(
                collisions, "self_distance_min", slider=True, text="Minimum Distance"
            )
            col.prop(
                collisions, "self_impulse_clamp", slider=True, text="Impulse Clamping"
            )
...
def soft_panel(layout, pi, mod):
    if ui_collapse_prop(
        layout, pi, "collapse_softbody", "Soft Body settings", icon="MOD_SOFT"
    ):
...
def collision_panel(layout, pi, mod):
    if ui_collapse_prop(
        layout, pi, "collapse_collisions", "Collisions settings", icon="MOD_PHYSICS"
    ):
        ...
        if obj:
            ...
            if res:
                return (
                    ...
                    and len([x for x in physics_settings.items if x.object])
                )
...
class PANEL_PT_MustardUI_Physics_Cache(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator(
            "mustardui.physics_bake_syncframes", text="", icon="UV_SYNC_SELECT"
        )
        row = box.row(align=True)
