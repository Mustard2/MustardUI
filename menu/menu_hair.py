...
class PANEL_PT_MustardUI_Hair(MainPanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        if arm is not None:
            ...
            hair_global_properties_avail = len(
    ...
    def draw(self, context):
        ...
        if rig_settings.hair_collection is not None:
            ...
            if hair_num > 0:
                try:
                    ...
                    if (
                        ...
                        row.separator()
                        if any(
                            "Dynamic" in x.particle_system.name
                            for x in mod_particle_system
                        ):
                            status = any(
                                x.particle_system.use_hair_dynamics
                                for x in mod_particle_system
                            )
                            op = row.operator(
                        ...
                        if rig_settings.hair_particle_collapse:
                            ...
                            for mod in mod_particle_system:
                                ...
                                row.label(
                                    text=format_dynamic_name(mod), icon="PARTICLES"
                                )
                    ...
                    if addon_prefs.developer:
                        box.label(
                            text="Enter and exit Configuration mode to fix.",
                            icon="BLANK1",
                        )
                        box.operator(
                ...
                if (
                    ...
                    or rig_settings.hair_enable_global_normalautosmooth
                ):
                    box = layout.box()
                    row = box.row(align=True)
            ...
            # Curves
            curves_hair = sorted(
                [x for x in rig_settings.hair_collection.objects if x.type == "CURVES"],
                key=lambda x: x.name,
            )
