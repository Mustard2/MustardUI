...
class MustardUI_Configuration(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not obj.MustardUI_enable:
            ...
            if not obj.MustardUI_created:
                if (
                    ...
                    self.report(
                        {"ERROR"},
                        "MustardUI - Be sure to select the armature Object in the "
                        "viewport before continuing.",
                    )
                    return {"FINISHED"}
            ...
            if (
                ...
                or rig_settings.model_body.scale[1] != 1.0
                or rig_settings.model_body.scale[2] != 1.0
            ):
                warnings = warnings + 1
                if addon_prefs.debug:
                    print(
                        "MustardUI - Configuration Warning - The selected body mesh "
                        "seems not to have the scale "
            ...
            if tools_settings.autoeyelid_enable:
                if (
                    tools_settings.autoeyelid_eyeL_shapekey == ""
                    and tools_settings.autoeyelid_eyeR_shapekey == ""
                ) and tools_settings.autoeyelid_driver_type == "SHAPE_KEY":
                    self.report(
                        {"ERROR"},
                        "MustardUI - At least one shape key should be selected if "
                        "Auto Blink tool is enabled.",
                    )
                    return {"FINISHED"}
                    ...
                    self.report(
                        {"ERROR"},
                        "MustardUI - At least one custom property should be selected "
                        "if Auto Blink tool is "
                        "enabled.",
                    )
                    return {"FINISHED"}
                    ...
                    try:
                        ...
                        self.report(
                            {"ERROR"},
                            "MustardUI - The custom property selected for Auto Blink "
                            "can not be found.",
                        )
                        return {"FINISHED"}
            ...
            if rig_recognized < 2:
                ...
                if addon_prefs.debug:
                    print(
                        "MustardUI - Configuration Warning - The rig has multiple "
                        "rig types. This might create "
                        "problems in the UI"
                    )
            ...
            if (
                ...
                try:
                    ...
                    if addon_prefs.debug:
                        print(
                            "MustardUI - Configuration Warning - The outfits_list "
                            "property index seems to be "
            ...
            if (
                ...
                if (
                    ...
                    try:
                        ...
                        if addon_prefs.debug:
                            print(
                                "MustardUI - Configuration Warning - The hair_list "
                                "property index seems to be "
                                "corrupted. Try to remove the UI and re-add it"
                            )
            ...
            if warnings > 0:
                ...
                if addon_prefs.debug:
                    print(
                        "MustardUI - Configuration Warning - No warning or errors "
        ...
        if (
            (
                settings.viewport_model_selection_after_configuration
                and not settings.viewport_model_selection
            )
            or (
                not settings.viewport_model_selection_after_configuration
                and settings.viewport_model_selection
            )
        ) and not obj.MustardUI_created:
