...
class MustardUI_CleanModel(bpy.types.Operator):
    ...
    remove_dangling_cp: BoolProperty(
        ...
        description="After removing the Outfit or Hair, the associated custom "
        "properties are deleted by default.\nUncheck this to keep the custom "
        "properties",
    )
    remove_dangling_pi: BoolProperty(
        default=True,
    ...
    def execute(self, context):
        ...
        # Check diffeomorphic custom morphs existence and delete all of them
        # (except JCMs)
        if self.remove_morphs:
            ...
            # Remove diffeomorphic support from the UI to avoid errors in the UI, or
            # restore it if FACS are asked
    ...
    def draw(self, context):
        ...
        if morphs_settings.enable_ui:
            ...
            if self.remove_morphs:
                ...
                col2.label(
                    text="Some bones of the Face rig might not work even if Remove "
                    "Face Rig Morphs is disabled!",
                    icon="BLANK1",
                )
            row = col.row()
