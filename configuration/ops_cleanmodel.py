...
class MustardUI_CleanModel(bpy.types.Operator):
    """Clean the model to get better performance, at the cost of deleting some
    features/shape keys/morphs/outfits"""
    ...
    def execute(self, context):
        ...
        # Check diffeomorphic custom morphs existence and
        # delete all of them (except JCMs)
        if self.remove_morphs:
            ...
            # or restore it if FACS are asked
    ...
    def draw(self, context):
        ...
        col.label(
            text="Do not use while producing, but before starting a project "
            "with the model.",
            icon="DOT",
        )
        col.label(
        ...
        if morphs_settings.enable_ui:
            ...
            if self.remove_morphs:
                ...
                col2.label(
                    text="Some bones of the Face rig might not work even if "
                    "Remove Face Rig Morphs is disabled!",
                    icon="BLANK1",
                )
            row = col.row()
            ...
            if self.remove_diffeomorphic_data:
                col2 = col.column(align=True)
                col2.label(
                    text="After cleaning, Diffeomorphic tools might now work for "
                    "this model!",
                    icon="ERROR",
                )
                col2.label(
                    text="Use this option when you are not planning to use "
                    "Diffeomorphic for this model anymore.",
                    icon="BLANK1",
                )
                col2.label(
