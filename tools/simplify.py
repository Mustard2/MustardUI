...
class MustardUI_SimplifySettings(bpy.types.PropertyGroup):
    ...
    simplify_normals_autosmooth: bpy.props.BoolProperty(
        name="Affect Normals Auto-Smooth", default=True
    )
    ...
    def execute(self, context):
        ...
        if simplify_settings.simplify_revert_settings:
            if "mustardui_pre_simplify" not in arm:
                ...
                outfits_global_normalautosmooth = (
                    rig_settings.outfits_global_normalautosmooth
                )
                ...
                arm["mustardui_pre_simplify"] = {
                    ...
                    "body_norm_autosmooth": body_norm_autosmooth,
                    ...
                    "outfits_global_normalautosmooth": outfits_global_normalautosmooth,
                    ...
                    "hair_global_normalautosmooth": hair_global_normalautosmooth,
                }
        ...
        if (
            simplify_settings.simplify_normals_optimize
            and simplify_settings.simplify_enable
        ):
        ...
        if simplify_settings.simplify_enable:
            ...
            if rig_settings.body_enable_norm_autosmooth:
                rig_settings.body_norm_autosmooth = (
                    not simplify_settings.simplify_enable
                )
        ...
        # Outfits
        if (
            len(rig_settings.outfits_list) > 1
            and simplify_settings.simplify_outfit_global
        ):
            if (
                ...
                rig_settings.outfits_global_subsurface = (
                    not simplify_settings.simplify_enable
                )
            ...
            if rig_settings.outfits_enable_global_shrinkwrap:
                rig_settings.outfits_global_shrinkwrap = (
                    not simplify_settings.simplify_enable
                )
            if rig_settings.outfits_enable_global_solidify:
                rig_settings.outfits_global_solidify = (
                    not simplify_settings.simplify_enable
                )
            if rig_settings.outfits_enable_global_triangulate:
                rig_settings.outfits_global_triangulate = (
                    not simplify_settings.simplify_enable
                )
            if (
                simplify_settings.simplify_normals_autosmooth
                and rig_settings.outfits_enable_global_normalautosmooth
            ):
                rig_settings.outfits_global_normalautosmooth = (
                    not simplify_settings.simplify_enable
                )
        if (
            ...
            and simplify_settings.simplify_outfit_switch_nude
            and simplify_settings.simplify_enable
        ):
        ...
        if rig_settings.extras_collection and simplify_settings.simplify_extras:
            ...
            for obj in items:
                if simplify_settings.simplify_enable:
                    status = (
                        obj.MustardUI_outfit_visibility
                    ...
                    not simplify_settings.simplify_enable
                    and obj.MustardUI_OutfitSettings.simplify_status
                ):
                    if (
                        obj.MustardUI_outfit_visibility
                        != simplify_settings.simplify_enable
        ...
        if rig_settings.hair_collection:
            rig_settings.hair_collection.hide_viewport = (
                simplify_settings.simplify_enable
                if simplify_settings.simplify_hair
                else False
            )
            ...
            if simplify_settings.simplify_hair_global:
                if (
                    simplify_settings.simplify_subdiv
                    and rig_settings.hair_enable_global_subsurface
                    and simplify_settings.simplify_enable
                ):
                    rig_settings.hair_global_subsurface = (
                        not simplify_settings.simplify_enable
                    )
                if rig_settings.hair_enable_global_smoothcorrection:
                    rig_settings.hair_global_smoothcorrection = (
                        not simplify_settings.simplify_enable
                    )
                if rig_settings.hair_enable_global_solidify:
                    rig_settings.hair_global_solidify = (
                        not simplify_settings.simplify_enable
                    )
                if (
                    simplify_settings.simplify_normals_autosmooth
                    and rig_settings.hair_enable_global_normalautosmooth
                ):
                    rig_settings.hair_global_normalautosmooth = (
                        not simplify_settings.simplify_enable
                    )
        ...
        if (
            not simplify_settings.simplify_enable
            and simplify_settings.simplify_revert_settings
        ):
            if "mustardui_pre_simplify" in arm:
        ...
        if simplify_settings.simplify_particles and simplify_settings.simplify_enable:
            ...
            if (
                ...
                and rig_settings.hair_enable_global_particles
            ):
                rig_settings.hair_global_particles = (
                    not simplify_settings.simplify_enable
                )
        ...
        if rig_settings.hair_collection:
            for obj in [
                x for x in rig_settings.hair_collection.objects if x in child_all
            ]:
                child.remove(obj)
        ...
        if physics_settings:
            for obj in [
                x.object for x in physics_settings.items if x.object in child_all
            ]:
                child.remove(obj)
        ...
        if morphs_settings and morphs_settings.enable_ui:
            if (
                simplify_settings.simplify_morphs_freeze
                and morphs_settings.enable_freeze_morphs
            ):
                if (
                    not simplify_settings.simplify_morphs
                    or morphs_settings.type == "GENERIC"
                ) and (
                    (
                        not morphs_settings.morphs_optimized
                        and simplify_settings.simplify_enable
                    )
                    or (
                        morphs_settings.morphs_optimized
                        and not simplify_settings.simplify_enable
                    )
                ):
                    bpy.ops.mustardui.morphs_optimize()
            elif (
                simplify_settings.simplify_morphs
                and "DIFFEO_GENESIS" in morphs_settings.type
            ):
                morphs_settings.diffeomorphic_enable = (
                    not simplify_settings.simplify_enable
                )

        # Physics
        if (
            simplify_settings.simplify_physics
            and physics_settings
        ...
        if (
            simplify_settings.simplify_force_no_physics
            and simplify_settings.simplify_enable
        ):
            for obj in context.scene.objects:
                for mod in [
                    ...
                    if x.type in ["SOFT_BODY", "CLOTH", "COLLISION"]
                ]:
                    ...
                    if addon_prefs.debug:
                        print(
                            f"MustardUI - Disabled {mod.type} modifier on: {obj.name}"
                        )

        # Force Disable Particles
        if (
            simplify_settings.simplify_force_no_particles
            and simplify_settings.simplify_enable
        ):
            for obj in context.scene.objects:
                for ps in [x for x in obj.modifiers if x.type == "PARTICLE_SYSTEM"]:
