    ...
    def blinkFrame(self, frame, value, blink_driver, obj, type):
        ...
        if type == "SHAPE_KEY":
            if blink_driver in obj.data.shape_keys.key_blocks.keys():
                ...
                obj.data.update_tag()
                obj.data.shape_keys.key_blocks[blink_driver].keyframe_insert(
                    data_path="value", index=-1, frame=frame
                )
                return False
            else:
                return True
            ...
            if blink_driver in obj.keys():
                ...
                obj.update_tag()
                obj.keyframe_insert(
                    data_path=f'["{blink_driver}"]', index=-1, frame=frame
                )
                return False
            else:
                return True
    ...
    def execute(self, context):
        ...
        if tools_settings.autoeyelid_driver_type == "SHAPE_KEY":
            for blink_driver in [
                ...
                tools_settings.autoeyelid_eyeR_shapekey,
            ]:
                if blink_driver != "":
            ...
            if frame % fps / 2 == 0:
                ...
                if r < blink_chance_per_half_second:
                    ...
                    for blink_driver in blink_drivers:
                        target_object = (
                            rig_settings.model_body
                            if tools_settings.autoeyelid_driver_type == "SHAPE_KEY"
                            else rig_settings.model_armature_object
                        )
                        error = error + self.blinkFrame(
                            blinkStart,
                            0.0,
                            blink_driver,
                            target_object,
        ...
        if error < 1:
            ...
            self.report(
                {"ERROR"},
                "MustardUI - Auto Blink shape keys/morph seems to be missing. Results "
                "might be corrupted.",
            )
