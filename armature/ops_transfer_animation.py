...
class MustardUI_Armature_TransferAnimation(bpy.types.Operator):
    """Copy all animation data (actions and NLA strip settings) and world transform from
    one armature to the Active Object armature"""
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"},
            f"MustardUI - Animation transferred from '{source.name}' to "
            f"'{target.name}'",
        )
        return {"FINISHED"}
