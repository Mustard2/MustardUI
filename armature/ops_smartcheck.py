...
class MustardUI_Armature_SmartCheck(bpy.types.Operator):
    """Check if a known armature type (ARP, Rigify, MHX) is available, and automatically
    set the Armature panel"""
    ...
    def execute(self, context):
        ...
        # MHX Rig
            ...
            if addon_prefs.debug:
                print(
                    "\nMustardUI - Smart Check - Found a MHX rig, setting layers for "
                    "MHX."
                )
            ...
            for o in coll.objects:
                for bcoll in obj.collections_all:
                    if bcoll.name == o.name:
                        ...
                        bcoll_settings.outfit_switcher_enable = True
                        bcoll_settings.outfit_switcher_collection = coll
                        bcoll_settings.outfit_switcher_object = o
                        if addon_prefs.debug:
                            print(
                                "\nMustardUI - Smart Check - Armature layer "
                                + bcoll.name
        ...
        if found_type != "" and not outfits:
            if found_colls > 0:
                self.report(
                    {"INFO"},
                    f"MustardUI - Smart Check found {found_colls} collections "
                    f"in a '{found_type}' armature.",
                )
            else:
                self.report(
                    {"WARNING"},
                    f"MustardUI - Smart Check found a '{found_type}' armature "
                    f"but nonviable collection.",
                )
        elif outfits:
            self.report(
